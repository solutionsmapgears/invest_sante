...
L.Map = L.Class.extend({
	...
	getPixelBounds: function () {
		var topLeftPoint = this._getTopLeftPoint(),
			size = this.getSize();
            //alert(this.layerPointToLatLng(this._initialTopLeftPoint));
            //var offset = L.DomUtil.getPosition(this._mapPane); alert(offset);
            //var topLeftPoint =  this.project(this.tileOrigin)._round();//.subtract(offset);;//._subtract(this._initialTopLeftPoint);
            //topLeftPoint.x -= this._initialTopLeftPoint.x;
            //alert(topLeftPoint.y);
...
/*
                if(this._map.tileOrigin){
                    tilePoint.x = tilePoint.x - Math.floor(this._map.project(this._map.tileOrigin)._round().x / this.options.tileSize);
                    tilePoint.y = Math.floor(this._map.project(this._map.tileOrigin)._round().y / this.options.tileSize) - tilePoint.y;
                }
*/
		...
		if (this.options.scheme === 'tms'/* && !this._map.tileOrigin*/) {
    ...
    _getTilePos: function (tilePoint) { 
	    ...
	    tileSize = this.options.tileSize;
            /*var t = new L.LatLng(this._map.tileOrigin.lng, 62.88);
            alert(this._map.project(t).y / tileSize);
            return new L.Point(tilePoint.x + 0.73, tilePoint.y - 0.6).multiplyBy(tileSize).subtract(origin);*/
